...
const payment = (function () {
     ...
     * 
     * @param {any} value numeric value that needs to rounded off
    ...
    /**
     * Get the remaining amount to be distributed
     * 
     * @param {any} paymentAmount payment amount that is being payed by the user
     * @param {any} principal principal balance
     * @param {any} interest interest balance
     * @param {any} fees fees
     * @returns the remaining amount
     */
    const getRemainingAmount = function (paymentAmount, principal, interest, fees) {
        return toFixed(parseFloat(paymentAmount) - (parseFloat(principal) + parseFloat(interest) + parseFloat(fees)));
    }

    /**
     * Get the total payment amount
     * 
     * @param {any} totalPrincipal the total principal balance
     * @param {any} totalInterest the total interest balance
     * @param {any} totalFees the total fees
     * @returns the total payment amount
     */
    const getTotalPayment = function (totalPrincipal, totalInterest, totalFees) {
        return toFixed(parseFloat(totalPrincipal) + parseFloat(totalInterest) + parseFloat(totalFees));
    }

    /**
     * Get the payment amount
     * 
     * @param {any} principal the principal balance
     * @param {any} interest the interest balance
     * @param {any} fees the fees
     * @returns the payment amount
     */
    const getPayment = function (principal, interest, fees) {
        return toFixed(parseFloat(principal) + parseFloat(interest) + parseFloat(fees));
    }
    ...
    /**
     * validate the maturity date for spread (called from portal).
     * 
     * @param {any} maturityDate the maturity date of the loan
     */
    const isCloseToMaturity = function (portalMaturityDate) {

        try {
            const maturityDate = new Date(portalMaturityDate);
            // JavaScript months are 0-indexed
            let maturityMonth = maturityDate.getMonth() + 1;
            ...
            if (
                (maturityYear === nextMonthYear && maturityMonth < nextMonthMonth) ||
                (maturityYear < nextMonthYear &&
                    maturityMonth < nextMonthMonth + 12)
            ) {
                return true;
            }
     ...
     * @param {any} maturityDate the maturity date of the loan
     */
    const processSpread = function (outstandingAmounts, paymentAmounts, total, maturityDate) {

        try {

            if (isCloseToMaturity(maturityDate)) {
                // spread fees
                if (total > 0) {
                    Object.keys(paymentAmounts).forEach(key => {
     ...
     * @param {any} maturityDate the maturity date of the loan
     */
    const spread = function (outstandingTableActor, paymentTableActor, paymentAmountActor, maturityDate) {
        let isTabular = true;
        ...
        processSpread(outstandingObject, paymentObject, amount, maturityDate);

        $callbackAction = $('[data-style-action-name="gmk_dealerPortalAutoFillCallRecordAction"]');
...
(function (portalext, $) {
    ...
    portalext.toFixed = payment.toFixed;
    portalext.getRemainingAmount = payment.getRemainingAmount;
    portalext.getTotalPayment = payment.getTotalPayment;
    portalext.getPayment = payment.getPayment;
