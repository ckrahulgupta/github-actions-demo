...
const changeLimitFieldColor = function(caller) {
    ...
    $dealerLoanLimitsHtml = $(`div[data-style-actor-name="gmk_dealerPortalLoanLimits"]`);
    ...
    if ($dealerLoanLimitsHtml.length != 0) {
        ...
        if (caller.value < caller.threshold) {
            ...
            if ($dealerLoanLimitsHtml.length != 0) {
                if (remainingContractLoanLimitTableHeadingHtml) {
                    remainingContractLoanLimitTableHeadingHtml.style.color = 'rgb(172 117 75)';
                }

                if (remainingContractLoanLimitValueHtml) {
                    remainingContractLoanLimitValueHtml.style.color = 'rgb(97 115 75)';
                }
            }
        }
    }
